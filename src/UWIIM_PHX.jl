...
function Q_a_CAP(x,p,t)
    Q_CAP = Q_s(x,p,t) - μ_max(p,2)
    short = μ_max(p,3) - Q_CAP
    NIA_avail = max(0,70022-short)
    high_avail = Q_CAP - NIA_avail
    
    return a_q(p,4)*NIA_avail + a_q(p,5)*high_avail
end;
...
function Q_a_SRP(x,p,t)
    max_prop_use = (D_bar(x,p,t)/η(x))*θ(p,2)
    NG = ζ(x)*a_q(p,6)*μ_max(p,2)
    max_allocation = μ_max(p,2)*a_q(p,2)
...
function P_proj(x,p,t) #project population out τ_p years
    ...
    κ_t = copy(κ(p)) #Assume same carrying capacity .
    
    return κ_t/(((κ_t-P_t)/P_t)*exp(-r(p)*τ_p(p)) + 1)
end;
...
function P_proj_1(x,p,t) #only project population out 1 year
    ...
    return κ_t/(((κ_t-P_t)/P_t)*exp(-r(p)) + 1)
end;
...
D(x,p,t) = χ(x)*P(x,p,t)*μ(x,p,t);

D_bar(x,p,t) = χbar(x)*P(x,p,t)*μ(x,p,t);
...
d_ST(x,p,t) = (χ(x)-u_1(x,p,t))*μ(x,p,t);
...
D_proj(x,p,t) = P_proj(x,p,t)*χbar(x)*μ(x,p,t)*(1-δ(p,1))^τ_p(p); #projects demand for τ_p years

D_proj_1(x,p,t) = P_proj_1(x,p,t)*χbar(x)*μ(x,p,t)*(1-δ(p,1)); #only projects demand for 1 year
...
Vbar_s(x,p,t) = υ_bar_s(x)*C_v_s(x)*μ_s(x);

Vbar_g(x,p,t) = υ_bar_g(x)*C_v_g(x)*μ_g(x);
...
V_s(x,p,t) = υ_s(x)*Vbar_s(x,p,t);

V_g(x,p,t) = υ_g(x)*Vbar_g(x,p,t);
...
function υ_and_η_proj_g(x,p,t)
    ...
    if(τ_p(p)>0)
        ...
        possibleInvests_η = floor(Int,min(τ_p(p),τ_i(p,2)-1)) 
        possibleInvests_w_s = floor(Int,min(τ_p(p),τ_i(p,4)-1)) 
        ...
        for y in 1:τ_p(p)
            ...
            if(possibleInvests_η>0)
                ...
                η_next = ifelse(η_next>η_max(p),η_max(p),η_next)
            else
                η_next = copy(η_prev)
            end
        ...
        υ_proj_t=max(100*a_q(p,1)*μ_max(p,1),(V_g(x,p,t) + Inflow*τ_p(p) - O_proj))/Vbar_g(x,p,t)
        η_proj_t=η_next
...
function A_l_s(x,p,t)
    ...
    return Q_a + a_v(p,2)*V_s(x,p,t)
end;

function A_l_g(x,p,t) 
    #Available Groundwater From Stored Water
    if(case(p)==1) #Phoenix Metro Area Case
        A_l_g_v = a_v(p,1)*max(0,V_g(x,p,t)-a_q(p,1)*μ_max(p,1)*100) ##only count available V if above 100-yr SY amount 
    else
        A_l_g_v = a_v(p,1)*V_g(x,p,t)
    end
    
    #Available Groundwater From Annual Inflow
    A_l_g_q = a_q(p,1)*Q_g(x,p,t);
...
A_w_s(x,p,t) = w_s(x)*(Vbar_s(x,p,t) + μ_s(x));

A_w_g(x,p,t) = w_g(x)*(Vbar_g(x,p,t) + μ_g(x));
...
function w_max_s(x,p,t)
    ...
    return W_max_s/(μ_s(x) + Vbar_s(x,p,t))
end;

function w_max_g(x,p,t)
    A_l_g_t = A_l_g(x,p,t)
    w_max_g = (A_l_g_t)/(Vbar_g(x,p,t)+μ_g(x)) #(taken out) 1.6 accounts for the desire to have surplus capacity for peak intrannual demands
...
function Vbar_proj_s(x,p,t)
    ...
    if(τ_p(p)==0) #no projection (τ_p = 0)
        return Vbar_t
    elseif(τ_i(p,3)==1) #immediate implementation (τ_i = 1)
        return Vbar_t
    else
        possibleInvests = floor(Int,min(τ_p(p),τ_i(p,3)-1)) #calculates the number of possible stored investments that need to be taken into account in the projection
...
function υbar_proj_s(x,p,t) #convert the projected surface storage capacity to υ non-dimensional units
    return Vbar_proj_s(x,p,t)/(C_v_s(x)*μ_s(x))
end;

#   Projected Supply Interconnect Capacity (\zeta^{proj})
#   ⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅⋅

function ζ_proj(x,p,t)
    ζ_t = ζ(x)
    ...
    elseif(τ_i(p,8)==1)
        ...
        possibleInvests = floor(Int,min(τ_p(p),τ_i(p,8)-1)) #calculates the number of possible stored investments that need to be taken into account in the projection
        if(possibleInvests==1)#if only implement 1 planned investment
            return ζ_t + x[plan_index_k(x,p,t)[8]]/H_ζ_max(p)
        else
            return ζ_t + sum(x[plan_index_k(x,p,t)[8]:(plan_index_k(x,p,t)[8]+possibleInvests-1)])/H_ζ_max(p)
        end
    end
end;
...
function w_proj_s(x,p,t)
    ...
    μ_t = copy(μ_s(x))
    μ_proj_t = μ_t
    A_w_t = (w_s(x)*(Vbar_t+μ_t))
    
    ##Determine the number of planned investments that will be implemented by the end of the projection period
    if(τ_p(p)==0) #no projection (τ_p = 0)
        return w_s(x)
    elseif(τ_i(p,4)==1) #immediate implementation (τ_i = 1)
        return A_w_t/(Vbar_proj_t + μ_proj_t)
    else
        possibleInvests = floor(Int,min(τ_p(p),τ_i(p,4)-1)) #calculates the number of possible stored investments that need to be taken into account in the projection
...
function w_proj_g(x,p,t)
    ...
    μ_t = copy(μ_g(x))
    μ_proj_t = μ_t
    A_w_t = (w_g(x)*(Vbar_t+μ_t))
    
    ##Determine the number of planned investments that will be implemented by the end of the projection period
    if(τ_p(p)==0) #no projection (τ_p = 0)
        return w_g(x)
    elseif(τ_i(p,5)==1) #immediate implementation (τ_i = 1)
        return A_w_t/(Vbar_proj_t + μ_proj_t)
    else
        possibleInvests = floor(Int,min(τ_p(p),τ_i(p,5)-1))  #calculates the number of possible stored investments that need to be taken into account in the projection
        if(possibleInvests==1)#only implement 1 planned investment
...
function w_max_g_proj(x,p,t)
    ...
    w_max_g = (A_l_g_t)/(Vbar_g(x,p,t)+μ_g(x)) 
...
function A_proj_l_s(x,p,t) 
    ...
    A_proj_l_sv = a_v(p,2)*υ_proj_s(x,p,t)*Vbar_proj_s(x,p,t)
    
    #Available Surface Water from Inflow
    if(case(p)==1) #Phoenix Metro Area Cases
        #CAP
        Q_CAP = μ_s(x) - μ_max(p,2)
        short = μ_max(p,3) - Q_CAP
        NIA_avail = max(0,70022-short)
        high_avail = Q_CAP - NIA_avail
        CAP_avail = a_q(p,4)*NIA_avail + a_q(p,5)*high_avail
        
        #SRP
        max_prop_use = (D_proj(x,p,t)/η_proj(x,p,t))*θ(p,2)
        NG = ζ_proj(x,p,t)*a_q(p,6)*μ_max(p,2)
        max_allocation = μ_max(p,2)*a_q(p,2)
        ...
        return  A_proj_l_sv + a_q(p,2)*μ_s(x)
    end
end; 
...
function A_proj_l_g(x,p,t) 
    ...
    if(β_c(p)==0)
        if(case(p)==1)
            A_l_g_v = p[13][1]*max(0,υ_and_η_proj_g(x,p,t)[1]*Vbar_g(x,p,t)-a_q(p,1)*μ_max(p,1)*100) ##only count available V if above 100-yr SY amount 
         else
...
A_proj_w_s(x,p,t) = w_proj_s(x,p,t)*(Vbar_proj_s(x,p,t)+μ_s(x));

A_proj_w_g(x,p,t) = w_proj_g(x,p,t)*(Vbar_g(x,p,t)+μ_g(x));
...
function O_1(x,p,t) 
    ...
    need = (D_ST(x,p,t)/η(x))*(1-θ(p,1))
    
    #calculate available water considering legal and technical constraints
    tech_avail = A_w_s(x,p,t)
    allocation = μ_max(p,2)*a_q(p,2)
    dem_op = (D_ST(x,p,t)/η(x))*θ(p,2) + ζ(x)*a_q(p,6)*μ_max(p,2) #on-project demand (demand that can use basic SRP rights) + annual NCS & gatewater 
...
R(x,p,t) = P(x,p,t)*π_max(p)*f(x)*(β_p(p)+((1-β_p(p))*(d_ST(x,p,t)/(χbar(x)*μ(x,p,t)))));
...
R_proj(x,p,t) = P_proj_1(x,p,t)*f(x)*π_max(p);
...
C_o(x,p,t) = g_o(p)*(P(x,p,t)^z_op(p))*(D_bar(x,p,t)^z_od(p));
...
C_o_proj(x,p,t) = g_o(p)*(P_proj_1(x,p,t)^z_op(p))*(D_proj_1(x,p,t)^z_od(p));
...
C_d(x,p,t) =J_b_avg(x)*(1+τ_b(p)*i_b(p));
...
function C_d_need(x,p,t) #needed debt service for the next year
    return J_b_avg(x)*(1+τ_b(p)*i_b(p)-(1/τ_b(p))-i_b(p)) + J_b(x,p,t)*((1/τ_b(p))+i_b(p))
end;
...
function Y_1(x,p,t)
    ...
    return 1/(1+exp(-λ(p,1)*(e_1t - ϵ(p,1))))
end;

function Y_2(x,p,t)
    e_2t = e_2(x,p,t)
    
    return 1/(1+exp(-λ(p,2)*(e_2t - ϵ(p,2))))
end;

function Y_3(x,p,t)
    e_3t = e_3(x,p,t)
    
    return 1/(1+exp(-λ(p,3)*(e_3t - ϵ(p,3))))
end;
...
function u_1(x,p,t)
    χ_min = d_min(p)/μ(x,p,t) #convert d_min to χ units
    
    u_1_t = χ(x)*Y_1(x,p,t)*α(p)*(1-(χ_min/χ(x)))
    
    if (u_1_t > (χ(x) - χ_min)) #if investment would push the per-capita demand below the minimum demand, lower the investment
        u_1_t = χ(x) - χ_min
    end
...
function u_m_η(x,p,t)
    #Note Current State
    x_t = ForwardDiff.value(η(x))
    A_t = ForwardDiff.value(A(x,p,t))
    
    #Decide on Goal State
    x_goal = ifelse(x_t>η_max(p), η_max(p), x_t) #If the current capacity is above maximum, allow the capacity to decay to maximum
    
    #Calculate Volumetric Investment Need
    u_m = (x_goal - x_t*(1-δ(p,2)))*A_t
...
function J_m_η(x,p,t)
    ...
    η_t = ForwardDiff.value(η(x))
    u_m = u_m_η(x,p,t)
    
    #Convert volumetric need to dollars 
    J_m = g(p,2)*((η_t*u_m)^z(p,2))
...
function u_m_v(x,p,t)
   #Note Current State
    x_t = ForwardDiff.value(υ_bar_s(x))
    c_v_t = ForwardDiff.value(C_v_s(x))
    μ_t = ForwardDiff.value(μ_s(x))
    x_max = ForwardDiff.value(υbar_max(p,2))
    ...
    if(β(p,3)==0)
        ...
        #Calculate volumetric investment need
        u_m = (x_goal - x_t*(1-δ(p,3)))*μ_t*c_v_t
...
function u_m_w_g(x,p,t)
   #Note Current State
    x_t = ForwardDiff.value(w_g(x))
    w_max = ForwardDiff.value(w_max_g(x,p,t))
    V_bar_g_t = ForwardDiff.value(Vbar_g(x,p,t))
    μ_g_t = ForwardDiff.value(μ_g(x))
    
    #Decide on goal state
    x_goal = ifelse(x_t>w_max, w_max, x_t) #If the current capacity is above maximum, allow the capacity to decay to maximum
    
    #Calculate volumetric investment need
    u_m = (x_goal - x_t*((1-δ(p,5))))*(V_bar_g_t + μ_g_t)
...
function J_k_e_need(x,p,t)
    ...
    for k in 2:8
        ...
        if(k==2)
            J_k[k] = g(p,k)*((β_k_t[k]*η_t*u_e_need_t)^z(p,k))
        else
            J_k[k] = g(p,k)*((β_k_t[k]*u_e_need_t)^z(p,k))
        end
    end
    
    ##Calculate remaining investment need for d_bar (long-term demand management)
    J_k[1] = g(p,1)*((((1-sum(β_k_t))*u_e_need_t)/γ(p,2))^z(p,1)) ###add consideration for gamma_L
...
function β_k(x,p,t)
    ...
    C_v_s_t = ForwardDiff.value(C_v_s(x))
    υ_bar_s_t = ForwardDiff.value(υ_bar_s(x))
    ...
    w_s_t = ForwardDiff.value(w_s(x))
    w_g_t = ForwardDiff.value(w_g(x))
    ...
    for k in 2:8
        if(β_k_t[k] > 0) #if it is currently being targeted for expansion investments
            ...
            if(k==2) #delivery efficiency
                u_max = A_t*(η_max(p) - η_proj_t)
            elseif(k==3) #storage capacity
                u_max = μ_t*C_v_s_t*(υbar_max(p,2) - υ_bar_proj_s_t)
            elseif(k==4) #surface processing
                u_max = (Vbar_s_t + μ_s_t)*(w_max_s_t - w_proj_s_t)
            elseif(k==5) #ground processing
                u_max = (Vbar_g_t + μ_g_t)*(w_max_g_t - w_proj_g_t)
            elseif(k==8)
                u_max = H_ζ_max(p)*(1-ζ_t)
            end
...
function J_bar(x,p,t)
    ...
    if(f(x) > (1-ψ(p,3))) #check to see if there is room to raise by ψ_r
        max_change = (1-f(x))/f(x)
    else
        max_change = ψ(p,3)
    end
    
    R_max = P_proj_1(x,p,t)*(1+max_change)*π_max(p)*f(x)
    
    #Calculate the Terms in the J_bar equation
    incr = (τ_b(p)*(R_max-C_o_proj(x,p,t)))/(γ(p,3)*(1+τ_b(p)*i_b(p)))
    
    NetOp = R(x,p,t)-C_o(x,p,t)

    J_bar = incr + NetOp - (1+i_b(p))*τ_b(p)*J_b_avg(x)
...
function u_2_k(x,p,t)
    ...
    for k in 1:8
        if(J_k_t[k]>0)
            if(k==2)
                u_2_k[k] = (1/η_t)*(((1/g(p,k))*(J_k_t[k]))^(1/z(p,k)))
            else
                u_2_k[k] = ((1/g(p,k))*(J_k_t[k]))^(1/z(p,k))
            end
        end
    end
...
function u_3(x,p,t)
    f_change = (1/(P(x,p,t)*π_max(p)))*C_d_need(x,p,t)*e_3(x,p,t)*Y_3(x,p,t)
    
    #Check for maximum rate increase limit
    if(f_change+f(x) > 1)
        f_change = 1 - f(x)
    elseif(f_change/f(x) > ψ(p,3))
        f_change = ψ(p,3)*f(x)
    end
...
function ImplementOrStoreLTInvest(x,u,p,t)
    ...
    for k in 1:length(u_impl_k)
        if(τ_i(p,k)==1) ####If τ_i is 1 there is no need for planning or separately accounting for maintenance 
            u_impl_k[k] = copy(u[k])
        else
            u_ready = ForwardDiff.value(x[counter+19]) #Note the investments that will be implemented in t 
            
            u_impl_k[k] = u_impl_k[k] + u_ready #implement the next investment in the queue
            
            if(τ_i(p,k)>2)
                for c in 3:τ_i(p,k) #move up past investments in the queue
                    u_next = ForwardDiff.value(x[counter+20])
...
function UpdateInfrastructure(x,u_impl,C_v_new,p,t) #H is the implemented H for that year
    ...
    I_t = [ForwardDiff.value(χbar(x))]
    push!(I_t, ForwardDiff.value(η(x)))
    push!(I_t, ForwardDiff.value(υ_bar_s(x)))
    push!(I_t, ForwardDiff.value(w_s(x)))
    push!(I_t, ForwardDiff.value(w_g(x)))
    push!(I_t, ForwardDiff.value(μ_s(x)))
    push!(I_t, ForwardDiff.value(μ_g(x)))
    push!(I_t, ForwardDiff.value(ζ(x)))
    I_new = copy(I_t)
    
    ###Note Other Variables that Have to be Adjusted for ForwardDiff
    C_v_old = [ForwardDiff.value(C_v_s(x)) ForwardDiff.value(C_v_g(x))]
    υ_bar_g_old = ForwardDiff.value(υ_bar_g(x))
    ...
    #########Inflow
    #Surface
    if(Δμ_type(p,2) == 2) #sudden change scenario
        if(case(p) == 1) #PMA Case
            I_new[6] = ifelse(t == Δμ_t(p,2)-1, (1+Δμ_pc(p,2))*(I_t[6]-μ_max(p,2)) + μ_max(p,2), I_t[6]) + u_k_t[6]
        else
            I_new[6] = ifelse(t == Δμ_t(p,2)-1, (1+Δμ_pc(p,2))*I_t[6], I_t[6]) + u_k_t[6]
        end
    elseif(Δμ_type(p,2)==1) #gradual change scenario
        if(t < Δμ_t(p,2)-1 || t >= Δμ_t(p,2)+Δμ_τ(p,2)-1)#if specified time for the gradual change to start has not occurred or has passed, do not change inflow
            I_new[6] = I_t[6] + u_k_t[6]
        else
            if(case(p)==1) #PMA Case
                b_g = (Δμ_pc(p,2)*μ_max(p,3))/Δμ_τ(p,2)
                
                I_new[6] = μ_max(p,2) + ((I_t[6]-μ_max(p,2)) + b_g) + u_k_t[6]
            else
                I_new[6] = I_t[6]*(-Δμ_pc(p,2)) + u_k_t[6]
            end
        end
    ...
    I_new[3] = υ_max_s_2*(1-δ(p,3)) + H_vbar_t2;
    ...
    I_new[4] = w_s_2*(1-δ(p,4)) + H_w_s_t2; #SW pumping capacity
    ...
    I_new[5] = w_g_2*(1-δ(p,5)) + H_w_g_t2; #GW pumping capacity
    
    ########Delivery Efficiency
    η_next=I_t[2]*(1-δ(p,2)) +  u_k_t[2]/A_t #Convert volumetric investment to delivery efficiency relevant units and add to existing delivery efficiency
    I_new[2]=ifelse(η_next>η_max(p),η_max(p),η_next)
    ...
    I_new[1] = χbar_2*(1-δ(p,1)) - H_dbar_t2;

    ########Supply Interconnection ζ
    I_new[8] = I_t[8] + u_k_t[8]/H_ζ_max(p)
...
function updateUsers(x,I_new,p,t)
    ...
    p_new = p_prop(x)*(1 + r(p)*(1-p_prop(x))) 
    
    ######Per-Capita Demand Dynamics
    d_ST_t = d_ST(x,p,t)
    χ_ST_t = d_ST_t/μ(x,p,t) #Convert d_ST to χ units
    χ_ST_2 = χ_ST_t*((μ_s(x)+μ_g(x))/(I_new[6]+I_new[7])) #Re-calibrate χ_ST to new streamflow context 
    χbar_2 = χbar(x)*((μ_s(x)+μ_g(x))/(I_new[6]+I_new[7])) #Re-calibrate χbar to new streamflow context
    
    χ_new = χ_ST_2*(1+α(p)*(1-(χ_ST_2/χbar_2))) #Calculate new PC demand in χ units 
    if(χ_new < d_min(p)/(I_new[6]+I_new[7])) #Check that new PC demand is above minimum per capita demand
        χ_new = d_min(p)/(I_new[6]+I_new[7])
    end
...
function runWaterBalance(x,C_v_new,I_new,υ_bar_g_new,p,t)
    ...
    #####Surface Storage 
    #Update
    υ_s_2 = υ_s(x)*((υ_bar_s(x)*C_v_s(x)*μ_s(x))/(I_new[3]*C_v_new[1]*I_new[6])) #Re-calibrate υ to new storage capacity and inflow context
    ...
    #####Ground Storage
    #Update
    υ_g_2 = υ_g(x)*((υ_bar_g(x)*C_v_g(x)*μ_g(x))/(υ_bar_g_new*C_v_new[2]*I_new[7])) #Re-calibrate υ to new storage capacity and inflow context
    ...
    ####Surface
    if(case(p)==1)#PMA Case, inflow is just the mean
        q_new_s=1
    else
        q_new_s = ρ(p,2)*(q_s(x)-1) + C_v_s(x)*sqrt(1-ρ(p,2)*ρ(p,2))*randn()+1;
    end
    ...
    if(case(p)==1)#PMA Case, inflow is just the mean
        ...
        q_new_g = ρ(p,1)*(q_g(x)-1) + C_v_g(x)*sqrt(1-ρ(p,1)*ρ(p,1))*randn()+1;
    end
...
function WaterSystem(x,u_t,p,t)
    ...
    xnew[19] = Infrast_update[1][8] #Supply Interconnection 
    ...
    xnew[18] = (x[18]*(τ_b(p)-1) + J_b(x,p,t))*(1/τ_b(p))
...
#   2.0 Call and Change Parameters and Initial Conditions
#   =====================================================

#   Call Parameters
#   –––––––––––––––

μ_max(p,i) = p[1][i] #[μ_s_max μ_g_max μ_other_1 μ_other_2]
π_max(p) = p[2] #2:π_max
ΔCv(p) = p[3] #3: ΔC_v = ΔC_v_scen 
ρ(p,i) = p[4][i] #ρ = [ρ_s, ρ_g]
κ(p) = p[5] #κ
r(p) = p[6] #Intrinsic Growth rate, r
m_i(p) = p[7] #7: Immigration Rate, m_i 
m_o(p) = p[8] #8: Emigration Rate, m_0 
h(p) = p[9] #9: Hedging Policy Coefficient, h 
υ_min(p,i) = p[10][i]  #10: Minimum Allowed Storage Level, υ_min 
η_max(p) = p[11] #11: Maximum Delivery Efficiency, η
υbar_max(p,i) = p[12][i] #12: Maximum Storage Capacity, υ_max [υ_bar_max_s υ_bar_max_g]
a_v(p,i) = p[13][i]  #13: Maximum Legal Allocation, Volume-related a_v
a_q(p,i) = p[14][i]  #14: Maximum Legal Allocation, Flow-related a_q  a = [a_gv a_gq a_sv a_sq a_q2 a_q3 a_q4 a_q5]
d_min(p) = p[15]       #15: Minimum Per-Capita Use, d_min
δ(p,k) = p[16][k]  #16: Infrastructure Decay, δ [δ_d δ_dbar δ_η δ_v δ_w_s δ_w_g]
τ_i(p,k) = p[17][k] #17: Implementation Time, τ_i [τ_d τ_η τ_v τ_w_s τ_w_g τ_μ_s τ_μ_g]
γ(p,j) = p[18][j] #18: Scope Rule, Controller Goals/References [γ_1 γ_2 γ_3]
ψ(p,j) = p[19][j]  #19: Choice Rules/Constraints [ψ_s ψ_r]
β(p,k) = p[20][k] #20: Investment Allocation Mental Model (β) [β_η β_v β_w_s β_w_g β_μ_s β_μ_g]
λ(p,j) = p[21][j] #21: Institutional Friction, Elasticity λ [λ_1 λ_2 λ_3]
ϵ(p,j) = p[22][j] #22: Institutional Friction, Costs ϵ [ϵ_1 ϵ_2 ϵ_3]       
g(p,k) = p[23][k] #23: Cost Function Coefficients, g [g_dbar g_η g_vbar g_ws g_wg g_μs g_μg]
z(p,k) = p[24][k] #24: Cost Function Scale Factors, z [z_dbar z_η z_vbar z_ws z_wg z_μs z_μg]
case(p) = p[25] #25: Select Case 
Δμ_type(p,i) = p[26][i] #26: Mean Inflow Change Scneario type, Δμ_type 
Δμ_pc(p,i) = p[27][i] #27: Mean Inflow Change Scneario percent change, Δμ_type 
Δμ_t(p,i) = p[28][i] #28: Mean Inflow Change Scneario time of change, Δμ_type 
Δμ_τ(p,i) = p[29][i] #29: Mean Inflow  Change Scneario duration of change, Δμ_type  [Δμ_s_type Δμ_s_pc Δμ_s_t Δμ_g_type Δμ_g_pc Δμ_g_t Δμ_s_τ Δμ_g_τ]
w_max(p,i) = p[30][i] #30: Maximum Pumping Capacity, w [w_max_s w_max_g]
θ(p,i) = p[31][i]  #31: Demand Proportions for Base Use, θ [θ_g θ_s1 θ_s2]
τ_m(p) = p[32] #32: Memory of Long-Term Integral Controller τ_m = 0 
τ_p(p) = p[33] #33: Projectioin in Long-Term Controller (τ_p) 
α(p) = p[34] #34: Short-term investment effectiveness coefficient, α
τ_b(p) = p[35] #35: Bond LIfe, τ_b
i_b(p) = p[36] #36: Bond Interest Rate, i_b
γ_3_abs(p) = p[37] #37: Whether γ_3 error is calculated with absolute value
β_c(p) = p[38] #38: Willingness to Cheat on Groundwater Allowance, β_c
ϕ(p,k) = p[39][k]  #39: Investment ($) Allocation - just used for g_k calculation [ϕ_η ϕ_v ϕ_w_s ϕ_w_g ϕ_μ_s ϕ_μ_g]
β_p(p) = p[40] #40: Proportion of the Expected Per Capita Revenue from Fixed Charges
g_o(p) = p[41] #41: Operating Cost Function Coefficient, g_o
z_op(p) = p[42] #42: Operating Cost Population Scale Factor, z_op
z_od(p) = p[43]; #43: Operating Cost Demand Scale Factor, z_od
H_ζ_max(p) = p[44]; #44: Maximum Investment in Drought Pipeline Capacity (for PHX case)
...
function μ_max!(p,i,val)
    p[1][i] = val
end
function π_max!(p,val) 
    p[2] = val
end
function ΔCv!(p,val) 
    p[3] = val
...
function υ_min!(p,i,val)
    p[10][i] = val 
end
function η_max!(p,val) 
    p[11] = val 
end
function υbar_max!(p,i,val) 
    p[12][i] = val 
end
function a_v!(p,i,val) 
    p[13][i] = val
end
function a_q!(p,i,val) 
    p[14][i] = val
end
function d_min!(p,val)
    p[15] = val
end
function δ!(p,k,val) 
    p[16][k]  
end
function τ_i!(p,k,val) 
    p[17][k] = val
end
function γ!(p,j,val)
    p[18][j]  = val
...
function Δμ_type!(p,i,val) 
    p[26][i] = val
end
function Δμ_pc!(p,i,val) 
    p[27][i] = val
end
function Δμ_t!(p,i,val) 
    p[28][i] = val
end
function Δμ_τ!(p,i,val) 
    p[29][i] = val
end
function w_max!(p,i,val) 
    p[30][i] = val
end
function θ!(p,i,val) 
    p[31][i]  = val
...
function p_prop_0!(x0,val) 
    x0[1] = val
end
function χ_0!(x0,val)
    x0[2] = val
end
function υ_s_0!(x0,val)
    x0[3] = val
end
function υ_g_0!(x0,val)
    x0[4] = val
end
function υ_bar_s_0!(x0,val)
    x0[5] = val
end
function υ_bar_g_0!(x0,val)
    x0[6] = val
end
function w_s_0!(x0,val)
    x0[7] = val
end
function w_g_0!(x0,val)
    x0[8] = val
end
function q_s_0!(x0,val)
    x0[9] = val
end
function q_g_0!(x0,val)
    x0[10] = val
end
function μ_s_0!(x0,val)
    x0[11] = val
end
function μ_g_0!(x0,val)
    x0[12] = val
end
function C_v_s_0!(x0,val)
    x0[13] = val
end
function C_v_g_0!(x0,val)
    x0[14] = val
end
function η_0!(x0,val)
    x[15] = val
end
function χbar_0!(x0,val)
    x[16] = val
end
function f_0!(x0,val)
    x[17] = val
end
function J_b_avg_0!(x0,val)
    x[18] = val
end
function ζ_0!(x0,val)
    x[19] = val
...
function Default(;μ_s1_max = 200000, μ_s2_max = 0, μ_g_max = 0.001, π_max = 1000, ΔC_v_scen = 0, ρ_s = 0.2, ρ_g = 0.0, κ = 1500000, r = 0.1, η_max = 1.7, υ_bar_max_s = 12.0, 
        υ_bar_max_g = 40.0, a_gv = 1.0, a_gq = 1.0, a_sv = 1.0, a_s1q = 1.0, a_s2q = 1.0, a_s3q = 1.0, a_s4q = 1.0, a_s5q = 1.0, d_min = 0.04, δ_dbar = 0.004, δ_v = 0.07, δ_η = 0.07, δ_w_s = 0.07, δ_w_g = 0.07, δ_ζ=0.07, τ_d = 1.0, 
        τ_v = 1.0, τ_η = 1.0, τ_w_s = 1.0, 
        τ_w_g = 1.0, τ_μ_s = 1.0, τ_μ_g = 1.0, τ_ζ=1.0, γ_1 = 1, γ_2 = 1.2, γ_3 = 2, ψ_r = 0.15, β_η = 0.3, β_v = 0.3, β_w_s = 0.3, β_w_g = 0.0, β_μ_s = 0.0, β_μ_g = 0.0, β_ζ = 0.0, λ_1 = 22.0, λ_2 = 22.0, λ_3 = 22.0, ϵ_1 = 0.0, ϵ_2 = 0.0, 
        ϵ_3 = 0.0, g_o = 1, g_dbar = 2000, g_η = 2000, g_vbar = 2000, g_ws = 2000, g_wg = 2000, g_μs = 2000, g_μg = 2000, g_ζ = 2000, z_op = 0.5, z_od = 1.03, z_dbar = 1, z_η = 1, z_vbar = 1, z_ws = 1, z_wg = 1, z_μs = 1, z_μg = 1, z_ζ = 1,
        case = 0, Δμ_s_type = 0, Δμ_s_pc = 0.0, Δμ_s_t = 0, Δμ_s_τ = 0, w_max_s = 1.0, w_max_g = 0.0, θ_g = 0, θ_s1 = 0, θ_s2 = 0, τ_p = 5, α = 0.5, τ_b = 15, i_b = 0.04, ϕ_η = 0.6, ϕ_v = 0.3, ϕ_w_s = 0.3, ϕ_w_g = 0.0, 
        ϕ_μ_s = 0.0, ϕ_μ_g = 0.0, ϕ_ζ = 0.0, β_p = 0.5, H_ζ_max = 0, p_0 = 0.65, χ_0 = 8.0E-7, υ_s_0 = 1.0, υ_g_0 = 1.0, υ_bar_s_0 = 4.0, υ_bar_g_0 = 1.0, w_s_0 = 1.0, w_g_0 = 0.0, q_s_0 = 1.0, q_g_0 = 0.0, μ_s_0 = 200000, μ_g_0 = 0.001, 
        C_v_s_0 = 0.1, C_v_g_0 = 0.01, η_0 = 1.0, χbar_0 = 8.0E-7, f_0 = 0.5, J_b_avg_0 = 69000000, ζ_0 = 0)
    
    ##########Parameters############
    #1:μ
    μbar = [μ_g_max μ_s1_max μ_s2_max]
    ...
    ###26-29: Mean Inflow Change Scneario Δμ
    Δμ_g_type = Δμ_g_pc = Δμ_g_t = Δμ_g_τ = 0 #not relevant in this model version
    #Type
    Δμ_type = [Δμ_g_type Δμ_s_type]
    #PC
    Δμ_pc = [Δμ_g_pc Δμ_s_pc]
    #time of shock
    Δμ_t = [Δμ_g_t Δμ_s_t]
    #duration
    Δμ_τ = [Δμ_g_τ Δμ_s_τ]
    ...
    #12: Maximum Storage Capacity, υbar_max
    υ_bar_max = [υ_bar_max_g υ_bar_max_s]
        
    ###13-14: Maximum Legal Allocation, a
    #Volume-based
    a_v = [a_gv a_sv]
    #Flow-based
    a_q = [a_gq a_s1q a_s2q a_s3q a_s4q a_s5q]
        
    #30: Maximum Pumping Capacity, w
    w_max = [w_max_g w_max_s]
        
    #####Water Use
    #15: Minimum Per-Capita Use, d_min
    
    #31: Demand Proportions for Base Use, θ
    θ = [θ_g θ_s1 θ_s2]
        
    #####Infrastructure Dynamic Characteristics
    #16: Infrastructure Decay, δ
    δ_μ_s = δ_μ_g = 0
    δ = [δ_dbar δ_η δ_v δ_w_s δ_w_g δ_μ_s δ_μ_g δ_ζ]
        
    #17: Implementation Time, τ_i
    τ_i = [τ_d τ_η τ_v τ_w_s τ_w_g τ_μ_s τ_μ_g τ_ζ]
        
    #####Institutional Context
    #18: Scope Rule, Controller Goals/References
    γ = [γ_1 γ_2 γ_3]
    
    #19: Choice Rules/Constraints
    ψ_s = ψ_l = 0 
    ψ = [ψ_s ψ_l ψ_r]
        
    #20: Investment Allocation Mental Model (β)
    β_dbar = 0
    β = [β_dbar β_η β_v β_w_s β_w_g β_μ_s β_μ_g β_ζ]
        
    #21-22: Institutional Friction
    λ = [λ_1 λ_2 λ_3]
    ϵ = [ϵ_1 ϵ_2 ϵ_3]
        
    #####Costs
    #23: Cost Function Coefficients, g
    g = [g_dbar g_η g_vbar g_ws g_wg g_μs g_μg g_ζ]

    #41: Operating Cost Coefficient, g_o
        
    #24: Cost Function Scale Factors, z
    z = [z_dbar z_η z_vbar z_ws z_wg z_μs z_μg z_ζ]

    #42: Operating Cost Population Scale Factor, z_op

    #43: Operating Cost Demand Scale Factor, z_od
    
    #34: Short-term investment effectiveness coefficient, α
    
    #35: Bond LIfe, τ_b
    
    #36: Bond Interest Rate, i_b
    
    #37: Whether γ_3 error is calculated with absolute value
    γ_3_abs = 0 #not relevant for this model version 
    
    #38: Willingness to Cheat on Groundwater Allowance, β_c
    β_c = 0 #not relevant in this model version
    
    #39: Investment ($) Allocation - just used for g_k calculation
    ϕ_dbar = 0
    ϕ = [ϕ_dbar ϕ_η ϕ_v ϕ_w_s ϕ_w_g ϕ_μ_s ϕ_μ_g ϕ_ζ]
    
    #40: Proportion of the Expected Per Capita Revenue from Fixed Charges
    
    
    #####Special Options
    #32: Memory of Long-Term Integral Controller
    τ_m = 0 #not relevant in this model version 
        
    #33: Projectioin in Long-Term Controller (τ_p) 
    
    #####Create Parameter Vector
    p = [μbar,π_max,ΔC_v,ρ,κ,r,m_i,m_o,h,υ_min,η_max,υ_bar_max,a_v,a_q,d_min,δ,τ_i,γ,ψ,β,λ,ϵ,g,z,case,Δμ_type,Δμ_pc,Δμ_t,Δμ_τ,w_max,θ,τ_m,τ_p,α,τ_b,i_b,γ_3_abs,β_c,ϕ,β_p,g_o,z_op,z_od,H_ζ_max]
    
    ##########Initial Conditions############
    #####Create Initial Conditions Vector
    x_0 = [p_0, χ_0,υ_s_0,υ_g_0,υ_bar_s_0,υ_bar_g_0,w_s_0,w_g_0,q_s_0,q_g_0,μ_s_0,μ_g_0,C_v_s_0,C_v_g_0,η_0,χbar_0,f_0,J_b_avg_0,ζ_0]
...
function set_g(μ_s_0,μ_g_0,μ_tot,μ_SRP,μ_CAP,a_SRP,a_CAP,a_SRP_NG,a_gq,a_gv,θ_s1,V_g_0, A_l_g_0,V_bar_g_0,
...
function Phoenix(;μ_SRP = 900000,  μ_CAP = 448663, μ_g_0 = 690602, π_max = 1000, κ = 1686528, r = 0.0878, η_max = 1.5, a_gv = 1, a_gq = 0.05357, a_sv = 0.0, a_SRP = 0.30883, 
        a_CAP = 0.41168, a_CAP_low=0.5324,a_CAP_high=0.3894,a_SRP_NG=0.06367, d_min = 0.072, δ=0.05, δ_dbar = 0.0003, τ=3.0, γ_2 = 1.1, γ_3 = 2, ψ_r = 0.15, β_η = 0.04, 
        β_w_s = 0, β_w_g = 0.21, β_ζ = 0.7, λ_1 = 110.0, λ_2 = 22.0, λ_3 = 22.0, ϵ_1 = 0.0, ϵ_2 = 0.0, ϵ_3 = 0.0, g_o = 0.1435, g_dbar = 5948, g_ζ = 5236,  z_op = 0.5581, z_od = 1.0303, z_dbar = 1, z_η = 1.01266,  
        z_w=1.04019, z_ζ = 1, Δμ_s_type = 2, Δμ_s_pc = -0.284, Δμ_s_t = 14, Δμ_s_τ = 0, θ_g = 0.024, θ_s1 = 0.5, τ_p = 5, α = 0.5, τ_b = 15, i_b=0.04, ϕ_η = 0.6605, ϕ_w_s = 0.2964, ϕ_w_g = 0.0331, 
        β_p = 0.6322, QC=0, A_SRP_0=142975.18, H_ζ_max = 57300, p_0 = 0.86466, χ_0 = 8.96274E-8, υ_g_0 = 0.53257, υ_bar_s_0 = 7.415E-9, w_s_0 = nothing, w_g_0 = 0.002843, C_v_s_0 = 0.001,C_v_g_0=0.001, 
        η_0 = 0.9877, χbar_0 = 8.96274E-8, f_0 = 0.23836, J_b_avg_0 = 69694375, ζ_0 = 0)
    ...
    τ_η =  τ_w_s = τ_w_g = τ_ζ = τ
    
    ####Hard Infrastructure Decay Rates, δ
    δ_η = δ_w_s = δ_w_g = δ_ζ = δ
    ...
    if(QC==1) #Queen Creek is calibrated to 2016 data to account for H20 aquisition
        ...
        g=set_g(μ_s_0,μ_g_0,μ_tot,μ_SRP,μ_CAP,a_SRP,a_CAP,a_SRP_NG,a_gq,a_gv,θ_s1,V_g_0, A_l_g_0, V_bar_g_0, 0,κ,p_0_QC,χbar_0_QC,
            χ_0_QC,f_0_QC,π_max,g_o,z_op,z_od,J_b_avg_0_QC,τ_b,i_b,η_0,w_g_0,w_s_0,δ_η,δ_w_s, δ_w_g,z_η, z_w, ϕ_η, (ϕ_w_s+ϕ_w_g),A_SRP_0)
    else
        g=set_g(μ_s_0,μ_g_0,μ_tot,μ_SRP,μ_CAP,a_SRP,a_CAP,a_SRP_NG,a_gq,a_gv,θ_s1,V_g_0, A_l_g_0, V_bar_g_0, 0,κ,p_0,χbar_0,χ_0,
    ...
    return Default(μ_s1_max=μ_SRP, μ_s2_max=μ_CAP, μ_g_max = μ_g_0, π_max = π_max, κ=κ, r = r, η_max = η_max, υ_bar_max_s = 6.45E-9, υ_bar_max_g = υ_bar_g_max, a_gv = a_gv, 
        a_gq = a_gq, a_sv = a_sv, a_s1q = a_SRP, a_s2q = a_CAP, a_s3q=a_CAP_low, a_s4q=a_CAP_high, a_s5q = a_SRP_NG,  d_min = d_min, δ_dbar = δ_dbar, δ_v = 0.0, δ_η = δ_η, δ_w_s = δ_w_s, 
        δ_w_g = δ_w_g, δ_ζ=δ_ζ, τ_d = 1.0, τ_η = τ_η, τ_w_s = τ_w_s, τ_w_g = τ_w_g, τ_ζ=τ_ζ, γ_2 = γ_2, γ_3 = γ_3, ψ_r = ψ_r, β_η = β_η, β_v = 0, β_w_s = β_w_s, β_w_g = β_w_g, β_ζ = β_ζ, β_μ_s = 0.0, β_μ_g = 0.0, 
        λ_1 = λ_1, λ_2 = λ_2, λ_3 = λ_3, ϵ_1 = ϵ_1, ϵ_2 = ϵ_2, ϵ_3 = ϵ_3, g_o = g_o, g_dbar = g_dbar, g_η = g_η, g_vbar = 0, g_μs = 0, g_μg = 0, g_ws = g_ws, g_wg = g_wg, g_ζ=g_ζ, z_op = z_op, 
        z_od = z_od, z_dbar = z_dbar, z_η = z_η, z_vbar = 1, z_ws = z_ws, z_wg = z_wg, z_μs = 1, z_μg = 1, z_ζ=z_ζ, case = 1, Δμ_s_type = Δμ_s_type, Δμ_s_pc = Δμ_s_pc, Δμ_s_t = Δμ_s_t, Δμ_s_τ = Δμ_s_τ,
        w_max_s = w_max_s, w_max_g = w_max_g, θ_g = θ_g, θ_s1 = θ_s1, τ_p = τ_p, α=α, τ_b = τ_b, i_b=i_b, ϕ_η = ϕ_η, ϕ_v = 0, ϕ_w_s = ϕ_w_s, ϕ_w_g = ϕ_w_g, ϕ_μ_s = 0, ϕ_μ_g = 0, β_p=β_p, 
        H_ζ_max = H_ζ_max,
        p_0 = p_0, χ_0 = χ_0, υ_s_0 = 0.0, υ_g_0 = υ_g_0, υ_bar_s_0 = υ_bar_s_0, υ_bar_g_0 = υ_bar_g_0, w_s_0 = w_s_0, w_g_0 = w_g_0, q_s_0 = 1.0, q_g_0 = 1.0, μ_s_0 = μ_s_0, μ_g_0 = μ_g_0, 
        C_v_s_0 = C_v_s_0, C_v_g_0 = C_v_g_0, η_0 = η_0, χbar_0 = χbar_0, f_0 = f_0, J_b_avg_0=J_b_avg_0, ζ_0 = ζ_0) 
end;
...
function Scottsdale(;π_max = 1000, κ = 242300, r = 0.143, η_max = 1.5, a_gv = 1, a_gq = 0.03114, a_sv = 0.0, a_SRP = 0.03067, a_CAP = 0.18076, a_CAP_low=0.0472, a_CAP_high = 0.2055,
        ...
        ϵ_3 = 0.0, g_o = 0.4316, z_op = 0.5581, z_od = 1.0303, z_η = 1.01266, z_w = 1.04019, Δμ_s_type = 2, Δμ_s_pc = -0.284, Δμ_s_t = 14, θ_g = 0.075, θ_s1 = 0.17, τ_p = 5, α = 0.5, 
    ...
    return Phoenix(π_max = π_max, κ=κ, r = r, η_max = η_max, a_gv = a_gv, a_gq = a_gq, a_sv = a_sv, a_SRP = a_SRP, a_CAP = a_CAP, a_CAP_low=a_CAP_low,a_CAP_high=a_CAP_high, a_SRP_NG=a_SRP_NG, 
        ...
        g_o = g_o, z_op = z_op, z_od = z_od, z_η = z_η, z_w = z_w, Δμ_s_type = Δμ_s_type, Δμ_s_pc = Δμ_s_pc, Δμ_s_t = Δμ_s_t, θ_g = θ_g, θ_s1 = θ_s1, τ_p = τ_p, α=α, τ_b = τ_b, i_b=i_b, 
...
function QueenCreek(;π_max = 1000, κ = 101553, r = 0.24, η_max = 1.5, a_gv = 1, a_gq = 0.02135, a_sv = 0.0, a_SRP = 0.0, a_CAP = 0.01038, a_CAP_low=0.0594, a_CAP_high = 0.0013,a_SRP_NG=0,
        ...
        g_o = 0.8346, z_op = 0.5581, z_od = 1.0303, z_η = 1.01266, z_w = 1.04019, Δμ_s_type = 2, Δμ_s_pc = -0.284, Δμ_s_t = 14, θ_g = 0.075, θ_s1 = 0.0, τ_p = 5, α = 0.5, τ_b = 15, 
...
function p2tuple(p::Array)
 ...
 function create_UWIIM(setup::Any=Default(); f=f)
    ...
    model = DeterministicIteratedMap(f,x_0,p_tup)
...
function createVarsDF(UWIIM,p,num_t,year_0)
   ##Generate Trajectory
    tr,t_range = trajectory(UWIIM, num_t)
    
    ##Collect Into DataFrame
    t = collect(t_range)
    col_copy = copy(collect(columns(tr)))

    p_ex, χ, υ_s, υ_g, υ_bar_s, υ_bar_g, w_s, w_g, q_s_ex, q_g_ex, μ_s, μ_g, C_v_s, C_v_g, η, χbar, f, J_b_avg, ζ_ex = columns(tr)
    ...
    for i in 1:length(tr.data)
        ...
        if(case(p)==1) 
            ...
            Q_1_ex[i] = μ_max(p,2)*a_q(p,2)
            
            Q_CAP = (Q_s(tr.data[i],p,t[i]) - μ_max(p,2))
            CAP_short = μ_max(p,3) - Q_CAP
            NIA_avail = max(0,70022-CAP_short)
            high_avail = Q_CAP - NIA_avail
            Q_2_ex[i] = a_q(p,4)*NIA_avail + a_q(p,5)*high_avail
        end
    end
    ...
    if(case(p)==1)
        vars = DataFrame(t=t,year=year_ex,p=p_ex, χ=χ, υ_s=υ_s, υ_g=υ_g, υ_bar_s=υ_bar_s, υ_bar_g=υ_bar_g, w_s=w_s, w_g=w_g, q_s=q_s_ex, q_g=q_g_ex, q=q_ex, Q_s=Q_s_ex, Q_g=Q_g_ex, Q=Q_ex, μ_s=μ_s, μ_g=μ_g, μ=μ_ex,
            ...
            A_l_s_proj=A_l_s_proj_ex, β_η=β_η_ex,β_w_s=β_w_s_ex,β_w_g=β_w_g_ex,Q_b=Q_b_ex,w_s_proj=w_s_proj_ex,ζ=ζ_ex,β_ζ=β_ζ_ex)
    else 
        vars = DataFrame(t=t,year=year_ex,p=p_ex, χ=χ, υ_s=υ_s, υ_g=υ_g, υ_bar_s=υ_bar_s, υ_bar_g=υ_bar_g, w_s=w_s, w_g=w_g, q_s=q_s_ex, q_g=q_g_ex, q=q_ex, Q_s=Q_s_ex, Q_g=Q_g_ex, Q=Q_ex, μ_s=μ_s, μ_g=μ_g, μ=μ_ex,
            ...
            u_e_need=u_e_need_ex,A_l_g_proj=A_l_g_proj_ex,A_g_proj=A_g_proj_ex,A_s_proj=A_s_proj_ex,η_proj=η_proj_ex,A_l_s_proj=A_l_s_proj_ex, β_η=β_η_ex,β_w_s=β_w_s_ex,β_w_g=β_w_g_ex,ζ=ζ_ex,β_ζ=β_ζ_ex)
    end
...
function timeSeriesPlot_nondim(vars,p,x_0)
    ...
    if(case(p)==1)
        ...
        plt_flows = plot(vars.t, [vars.q_a vars.Q_a_s./vars.μ a_q(p,2).*(1 .+ vars.C_v_s) a_q(p,2).*(1 .- vars.C_v_s) vars.Q_a_g./vars.μ vars.o_d vars.o_s vars.o_g vars.o_f], 
    ...
    plt_stor = plot(vars.t, [vars.υ_s.*vars.υ_bar_s vars.υ_bar_s], labels = ["Stor_Vol" "Stor_Capac"], ylims = (0, υbar_max(p,2).*1.5), xlabel = "Year", ylabel="Vol/(μ*C_v)", title = "Reservoir Storage", 
    ...
    #Financial Flows
    plt_fin = plot(vars.t,[vars.R.*0.000001 vars.C_o.*0.000001 vars.C_d.*0.000001 vars.J.*0.000001./τ_i(p,2)], labels = ["Rev" "Op Costs" "Debt Serv" "Inv Long"], 
        xlabel="Year", ylabel = "Dollars (M)", title = "Financial Flows", legend=:outerright, ylims=(0,(π_max(p)*κ(p).*0.000001)), 
    ...
    hline!([η_max(p)], labels="η_max", linestyle=:dash, linecolor=:purple)
    
    #Per-Capita Demand
    plt_d = plot(vars.t, [vars.χ vars.χbar (vars.u_1./vars.μ).*(-1) (vars.u_impl_dbar./vars.μ).*(-1) d_min(p)./vars.μ], xlabel = "Year", ylabel="Flows/cap/μ",
    ...
    μ_s_max = μ_max(p,2) + μ_max(p,3)
    
    hline!([μ_s_max/vars.μ[1]], labels="μ_s_max", linestyle=:dash, linecolor=:green)
    hline!([μ_max(p,1)/vars.μ[1]], labels="μ_g_max", linestyle=:dash, linecolor=:turquoise)
    
    #Population
    plt_P = plot(vars.t, vars.P.*0.001, ylabel="1000 Persons", xlabel = "Year", title = "Population", labels="Pop", ylims=(0,κ(p)*1.2*0.001), 
...
function timeSeriesPlot_dim(vars,p,x_0,units)
    ...
    if(case(p) == 1)
        ...
        if(units == "AF")
            plt_flows = plot(vars.year, [vars.Q_a.*0.001 vars.Q_a_s.*0.001 (1 .+ vars.C_v_s).*vars.μ_s.*a_q(p,2).*0.001 (1 .- vars.C_v_s).*vars.μ_s.*a_q(p,2).*0.001 vars.Q_a_g.*0.001 (1 .+ vars.C_v_g).*vars.μ_g.*a_q(p,1).*0.001 (1 .- vars.C_v_g).*vars.μ_g.*a_q(p,1).*0.001 vars.O_d.*0.001 vars.O_s.*0.001 vars.O_g.*0.001 vars.O_f.*0.001], 
            ...
            plt_flows = plot(vars.year, [vars.Q_a.*0.000000001 vars.Q_a_s.*0.000000001 (1 .+ vars.C_v_s).*vars.μ_s.*a_q(p,2).*0.000000001 (1 .- vars.C_v_s).*vars.μ_s.*a_q(p,2).*0.000000001 vars.Q_a_g.*0.000000001 (1 .+ vars.C_v_g).*vars.μ_g.*a_q(p,2).*0.000000001 (1 .- vars.C_v_g).*vars.μ_g.*a_q(p,1).*0.000000001 vars.O_d.*0.000000001 vars.O_s.*0.000000001 vars.O_g.*0.000000001 vars.O_f.*0.000000001], 
    ...
    #Storage Volume & Capacity
    if(case(p) ==1)
        SY = a_q(p,1)*μ_max(p,1)*100 #calculate the safe-yield storage volume
        ...
        if(units == "AF")
            plt_stor = plot(vars.year, [vars.V_s.*0.000001 vars.Vbar_s.*0.000001 vars.V_g.*0.000001 vars.Vbar_g.*0.000001], labels = ["Stor_Vol" "Stor_Capac" "Stor_Vol" "Stor_Capac"], 
                ylims = (0,max(υbar_max(p,2)*vars.C_v_s[1]*vars.μ_s[1], υbar_max(p,1)*vars.C_v_g[1]*vars.μ_g[1])*0.000001*1.5), xlabel = "Year", ylabel = "MAF", title = "Storage", 
            ...
            plt_stor = plot(vars.year, [vars.V_s.*0.000000001 vars.Vbar_s.*0.000000001 vars.V_g.*0.000000001 vars.Vbar_g.*0.000000001], labels = ["Stor_Vol" "Stor_Capac" "Stor_Vol" "Stor_Capac"], 
                ylims = (0,max(υbar_max(p,2)*vars.C_v_s[1]*vars.μ_s[1], υbar_max(p,1)*vars.C_v_g[1]*vars.μ_g[1])*0.000000001*1.5), xlabel = "Year", ylabel = "Bgal", title = "Storage", 
                legend = :outerright, linecolor = [:grey :black :grey :black], legendtitle="Storage Capacity")
        end
    end
    ...
    #Rate-Setting
    plt_f = plot(vars.year,vars.f.*π_max(p), labels = "f", xlabel="Time(years)", ylabel = "Per-Capita Rates (Dollars/yr)", title = "Rates - Max Rates Ratio", legend=:outerright, 
        ylims=(0,1.2*maximum(vars.f.*π_max(p))), linecolor = :brown, legendtitle="Rates")
    hline!([π_max(p)], labels = "Rate_PC_max", linestyle=:dash, linecolor=:brown) 
    if(case(p)==1)
        vline!([year_CAP],labels="t_CAP",linestyle=:dash, linecolor = :magenta)
    end
    ...
    plt_η = plot(vars.year, [vars.η vars.u_impl_η./vars.A], xlabel = "Year",labels = ["η" "Invest_η"],ylabel="Del. Eff (%Outflow)", title = "Delivery Efficiency", 
        linecolor = :purple, linestyle = [:solid :dot], legend=:outerright, ylims = (0,η_max(p)*1.2), legendtitle="Delivery Efficiency")
    hline!([η_max(p)], labels="η_max", linestyle=:dash, linecolor=:purple)
    if(case(p)==1)
        vline!([year_CAP],labels="t_CAP",linestyle=:dash, linecolor = :magenta)
    end
    ...
    if(units == "AF")
        ...
        hline!([d_min(p)*892.15], labels="Dem_PC_min", linestyle=:dash, linecolor=:indigo)
    else
       plt_d = plot(vars.year, [vars.d_ST./365 vars.χbar.*vars.μ./365 vars.u_1.*(-1)./365 vars.u_impl_plan_dbar.*(-1)./365], xlabel = "Year", ylabel="PC Dem (GPCD)", 
            ...
            linestyle = [:solid :solid :dot :dot], legendtitle="Per-Capita Demand")
        hline!([d_min(p)/365], labels="Dem_PC_min", linestyle=:dash, linecolor=:indigo)
    end
    if(case(p)==1)
        vline!([year_CAP],labels="t_CAP",linestyle=:dash, linecolor = :magenta)
    end
    ...
    if(units == "AF")
        plt_w = plot(vars.year, [vars.w_s.*(vars.υ_bar_s.*vars.C_v_s .+ 1).*vars.μ_s.*0.001 vars.w_g.*(vars.υ_bar_g.*vars.C_v_g .+ 1).*vars.μ_g.*0.001 vars.u_impl_w_s.*0.001 vars.u_impl_w_g*0.001 vars.w_max_s.*(vars.υ_bar_s.*vars.C_v_s .+ 1).*vars.μ_s.*0.001 vars.w_max_g.*(vars.υ_bar_g.*vars.C_v_g .+ 1).*vars.μ_g.*0.001], 
            ...
            linestyle = [:solid :solid :dot :dot :dash :dash], legend=:outerright, ylims = (0, min(maximum(vars.A_l_s*1.2*0.001),maximum(vars.A_l_g.*1.2*0.001))), legendtitle="Processing Capacity")
    else
    ...
    if(units=="AF")
        ...
        legend = :outerright, ylims = (0,max(μ_s_max,μ_max(p,1))*1.2*0.001), legendtitle="Mean Inflow")
    else
        plt_μ = plot(vars.year, [vars.μ_s.*0.000000001 vars.μ_g.*0.000000001], xlabel = "Year", labels = ["μ_s" "μ_g"], ylabel = "Mean Inflow (Bgal.yr)", title = "Mean Inflow", linecolor = [:darkgreen :turquoise], 
            linestyle = :solid, legend = :outerright, ylims = (0,max(μ_s_max,μ_max(p,1))*1.2*0.000000001), legendtitle="Mean Inflow")
    end
    if(case(p)==1)
        vline!([year_CAP],labels="t_CAP",linestyle=:dash, linecolor = :magenta)
    end
    ...
    plt_P = plot(vars.year, vars.P.*0.001, ylabel="Persons (K)", xlabel = "Year", title = "Population", labels="Pop", 
        ylims=(0,κ(p)*1.2*0.001), linecolor = :orange, linestyle = [:solid :dash :dot], legend = :outerright, legendtitle="Population")
    if(case(p)==1)
        vline!([year_CAP],labels="t_CAP",linestyle=:dash, linecolor = :magenta)
    end
